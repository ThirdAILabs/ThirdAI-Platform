...
for i in range(0, 100, 3):
    chunks.append(pdf.table.df["para"][i] + pdf.table.df["para"][i + 1] + pdf.table.df["para"][i + 2])
...
for chunk in tqdm.tqdm(chunks):
    qs = ask_gpt(f"Can you please generate 3 different questions that can be answered with the following context. Make sure to ONLY return the three with no fluff. No numbers and only separated by new lines. Thanks GPT! Here is the context: {chunk}")
    for q in qs.split("\n"):
        questions.append(q)
...
final_questions = unperturbed_in_cache + perturbed_not_in_cache + unperturbed_not_in_cache

df = pd.DataFrame({"in_cache": unperturbed_in_cache, "perturbed": perturbed_not_in_cache, "new_queries": unperturbed_not_in_cache})
